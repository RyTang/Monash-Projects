...
from preprocess.descriptors import load_annotation_json
...
def train(num_classes: int, num_epochs: int,
    ...
    #train_weights = train_dataset.dataset.weight_lst
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, collate_fn=utils.collate_fn)
    ...
    #                                               step_size=3,
    #                                               gamma=0.1)
    #lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max = batch_size)
...
if __name__ == "__main__":
    ...
    train(num_classes=len(le.classes_),
          ...
          descriptor="slogans", num_epochs=7)
